...
export default function AreasRegistradasDnD(props) {
  ...
  function onDragEnd(result) {
    ...
    const items = reorder(areas, result.source.index, result.destination.index);
    ...
    const post = newList.map((item) => ({
      ...
      orderIndex: newList
        ...
        .indexOf(item.orderIndex)
        .toString(), ////mejorar esto
      type: item.type,
    }));
  ...
  async function sendToAPI(id, name) {
    ...
    try {
      ...
      fetch(
        `https://qxtbqbuj4m.execute-api.us-east-1.amazonaws.com/prod/propertyareas/${propertyId}`,
        {
          method: "POST",
    ...
    return (
      <>
        ...
        <div style={{ paddingLeft: "10px" }}>
          <h4>
            PROTOCOLO DE DESINFECCIÓN: Prevención de contaminación cruzada{" "}
            <br />
          </h4>
          <h5>
